...
const MainInput = ({ label, placeholder, }: { label: string, placeholder: string }) => {
    return (
        <Field>
            ...
            <Input
                className={clsx(
                    'mt-1 block w-full rounded-lg border border-tertiary/20 py-2 px-3 text-secondary bg-[#F5F5F8]',
                    'focus:outline-none data-[focus]:outline data-[focus]:-outline-offset-2 data-[focus]:outline-white/25'
                )}
                placeholder={placeholder}
            />
            {/* <Description className="text-red-500 text-sm/6">Use your real name so people will recognize you.</Description> */}
        </Field>
