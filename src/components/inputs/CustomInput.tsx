...
const CustomInput: React.FC<CustomInputProps> = ({ label, name, control, rules, type, variant = 'primary', ...props }) => {
    ...
    const baseStyles = ' rounded-lg py-2 pl-8 px-3 w-full focus:outline-none data-[focus]:outline data-[focus]:-outline-offset-2 data-[focus]:outline-white/25 mt-1 block cursor-text';
    const styles = {
    ...
    return (
        <>
            <Field className="mb-2">
                <Label className="text-base font-semibold capitalize text-[#333333]" htmlFor={name}>{label}</Label>

                <div className='relative'>
                    <span className='absolute inset-0 top-3 w-fit'>
